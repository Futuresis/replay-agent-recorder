...
This file preserves the content and lower-level notes from the previous top-level `README.md` before the public-facing README refresh.

The new top-level README intentionally focuses on the value proposition, the no-API-key demo, the public API, and navigation. Detailed implementation notes from the old README are preserved here so they are not lost.
